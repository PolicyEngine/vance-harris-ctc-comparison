...
def calculate_ctc(situation, reform=None, vary_earnings=False):
    if reform is "baseline":
        ...
        num_children = sum(1 for person in situation["people"].values() if person.get("age", {}).get(YEAR, 100) < 17)
        return VANCE_AMOUNT * num_children
    elif reform is "vance_non_refundable":
        return Simulation(situation=situation, reform=vance_non_refundable).calculate("ctc_value", YEAR)[0]
    elif reform is "harris":
        return Simulation(situation=situation, reform=harris).calculate("refundable_ctc", YEAR)[0]
    else:
