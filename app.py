...
import pandas as pd
from utils import LIGHT_GRAY, DARK_RED, LIGHT_RED, BLUE, create_situation, calculate_ctc, MAX_INCOME, YEAR, TEAL_ACCENT
...
def create_reform_comparison_graph(df):
    ...
    baseline_value = df_sorted[df_sorted['reform'] == 'Baseline']['ctc'].values[0]
    ...
    fig.update_layout(
        title=dict(text="How reforms would affect your child tax credit", font=dict(size=24)),
...
child_ages = [st.number_input(f"Age of child {i+1}", min_value=0, max_value=16, value=5) for i in range(num_children)]
earnings = st.number_input(f"Household wages and salaries in {YEAR}", min_value=0, max_value=int(MAX_INCOME), value=50000, step=1000)
...
if st.button("Generate Comparison"):
    ...
    for reform_key, reform_name in reforms:
        ...
        df = df.append({'reform': reform_name, 'ctc': ctc_value, 'earnings': earnings}, ignore_index=True)
        
        # Update the headline and chart
        if reform_key == "baseline":
            headline_placeholder.markdown(f"<h1 style='text-align: center;'>In {YEAR}, you are eligible for a <br><span style='color:{TEAL_ACCENT};'>${ctc_value:,.0f}</span> child tax credit.</h1>", unsafe_allow_html=True)
